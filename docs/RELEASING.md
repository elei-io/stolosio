...
`v0.1.8` publishes image tags `0.1.8`, `0.1`, `sha-<short-commit>`, and `latest`, then
publishes the Helm chart as OCI version `0.1.8`. Pre-release tags do not move `latest`.
...
To release `0.1.8` after CI passes:
