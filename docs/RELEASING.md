...
`v0.1.5` publishes image tags `0.1.5`, `0.1`, `sha-<short-commit>`, and `latest`, then
publishes the Helm chart as OCI version `0.1.5`. Pre-release tags do not move `latest`.
...
To release `0.1.5` after CI passes:
