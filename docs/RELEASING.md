...
`v0.1.3` publishes image tags `0.1.3`, `0.1`, `sha-<short-commit>`, and `latest`, then
publishes the Helm chart as OCI version `0.1.3`. Pre-release tags do not move `latest`.
...
To release `0.1.3` after CI passes:
