...
GATEWAY_ACTIVE = Gauge(
    ...
    "Unexpired logical sessions consuming Stolosio intake capacity.",
    registry=REGISTRY,
)
...
COMMAND_DURATION = Histogram(
    "stolosio_command_duration_seconds",
...
NATS_CONNECTED = Gauge(
    "stolosio_nats_connected",
    "Whether this Stolosio process currently has a usable NATS connection.",
    registry=REGISTRY,
)
JETSTREAM_TOPOLOGY_READY = Gauge(
    "stolosio_jetstream_topology_ready",
    "Whether Stolosio's application-owned JetStream topology is available.",
    registry=REGISTRY,
)
JETSTREAM_REHYDRATED_EVENTS = Counter(
    "stolosio_jetstream_rehydrated_events",
    "PostgreSQL event records republished after recreating Stolosio's event stream.",
    registry=REGISTRY,
)
EVENT_RECORDER_EVENTS = Counter(
    "stolosio_event_recorder_events",
    "Recorder inputs by persistence outcome.",
    ("outcome",),
    registry=REGISTRY,
)
EVENT_RECORDER_LAG = Gauge(
    "stolosio_event_recorder_lag_seconds",
    "Age of the newest event received by the recorder.",
    registry=REGISTRY,
)
EVENT_RECORDER_PENDING = Gauge(
    "stolosio_event_recorder_pending",
    "Events pending on the recorder durable consumer.",
    registry=REGISTRY,
)
EVENT_DEAD_LETTERS = Counter(
    "stolosio_event_dead_letters",
    "Events terminated into the sanitized dead-letter stream.",
    ("reason",),
    registry=REGISTRY,
)
RETENTION_DELETED_ROWS = Counter(
    "stolosio_retention_deleted_rows",
    "Rows removed by bounded retention work.",
    ("table",),
    registry=REGISTRY,
)
RETENTION_DURATION = Histogram(
    "stolosio_retention_duration_seconds",
