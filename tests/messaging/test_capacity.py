...
async def test_polling_notifier_waits_without_external_state() -> None:
    ...
    await notifier.wait(ProviderName.BROWSERLESS, 0.01)
...
async def test_nats_capacity_notification_wakes_waiter() -> None:
    ...
    try:
        waiter = asyncio.create_task(notifier.wait(ProviderName.BROWSERLESS, 1))
        await asyncio.sleep(0.01)
        await notifier.notify(ProviderName.BROWSERLESS)
